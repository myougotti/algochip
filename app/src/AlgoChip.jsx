...
import { makeArray, SORT_GENERATORS } from "./sortSteps";
...
export default function AlgoChip() {
  ...
  // Sync snapshot refs when the seeded array changes (mount and Shuffle).
  // Counter/status state resets happen in handleShuffle, the only event that
  // changes the seed; effects only touch refs here. Declared BEFORE the paint
  // effect so refs are fresh by the time the paint effect runs on the same
  // render.
  useEffect(() => {
